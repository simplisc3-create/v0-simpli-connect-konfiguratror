...
export function validateConfig(configRaw: Config): RuleMessage[] {
  ...
  if (cfg.panels?.shelves && cfg.panels.shelves > 0 && cfg.panels.shelves < d.compartments) {
    msgs.push({
      code: "SHELVES_LESS_THAN_COMPARTMENTS",
      severity: "warning",
      message: `Du hast weniger Böden (${cfg.panels.shelves}) als Fächer (${d.compartments}). Das erzeugt offene Fächer.`,
    })
  }
  ...
  if (d.totalPanels > 0) {
    msgs.push({
      code: "PANELS_PACKING",
      severity: "info",
      message: `Flächen werden als 2er-Pack geliefert. Benötigt: ${d.totalPanels}, geliefert: ${d.panelPacks * 2}.`,
    })
  }
  ...
  if (cfg.material === "metal" && cfg.width === 75 && d.extraMetalScrewsQty > 0) {
    msgs.push({
      code: "METAL_EXTRA_SCREWS",
      severity: "info",
      message: `80er Breite benötigt zusätzliche Sicherung: ${d.extraMetalScrewsQty} Zusatzschrauben werden ergänzt.`,
    })
  }
  ...
  if (cfg.material === "glass") {
    ...
    if (cfg.width === 75) {
      msgs.push({
        code: "GLASS_STAB_80",
        severity: "info",
        message: `Bei Breite 75/80 wird pro Fach+Ebene ein Stabilisierungsstab ergänzt (${d.stabilizerRodQty} Stück).`,
      })
    }
  }

  msgs.push({
    code: "AUTO_PARTS_INFO",
    severity: "info",
    message: `Automatisch berechnet: ${d.uprightsQty} Leitern, ${d.tubeSetQty} Stangensets, ${d.adaptersQty} Adapter, ${d.screwSetQty} Schraubenset(s).`,
  })

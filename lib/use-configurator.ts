...
import type { Config, RuleMessage, BomLine, Derived } from "./rules-types"
import { normalizeConfig, derive, validateConfig, generateBOM, mapWidthToERP } from "./rules-engine"
...
export function useConfigurator(opts: UseConfiguratorOptions): UseConfiguratorReturn {
  ...
  const exportCsv = useCallback(() => {
    ...
    const rows = bom.map((l) => [l.sku, l.name, String(l.qty), l.unit, l.category, l.note ?? ""])

    // CSV safe escaping - wrap in quotes and escape internal quotes

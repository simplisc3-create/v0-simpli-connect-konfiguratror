...
import { ArrowLeft, ShoppingCart, Package, Check, Truck, RotateCcw, Award, ChevronRight, Archive, Sparkles, Maximize, Layers } from "lucide-react"
...
const product = {
  ...
  name: "Mit Schubladen 80",
  fullName: "Modul mit zwei Vollauszug-Schubladen",
  artNr: "MOD-80-008",
  description: "Das Premium-Schubladenmodul mit zwei geräumigen Vollauszug-Schubladen. Perfekt für Kleidung, Dokumente oder Kleinteile. Die Soft-Close-Funktion sorgt für sanftes, leises Schließen. Die Schubladen lassen sich vollständig herausziehen für optimalen Zugang.",
  shortDescription: "Optimaler Stauraum mit Vollauszug.",
  price: 85.0,
  category: "80er",
  moduleType: "mit-doppelschublade",
  width: 80,
  features: [
    { icon: Archive, title: "Zwei Schubladen", description: "Doppelter Stauraum für Organisation" },
    { icon: Maximize, title: "Vollauszug", description: "100% Zugang zum Inhalt" },
    { icon: Sparkles, title: "Soft-Close", description: "Sanftes, leises Schließen" },
    { icon: Layers, title: "Stapelbar", description: "Beliebig vertikal erweiterbar" },
  ],
  specs: [
    { label: "Breite", value: "80 cm" },
    { label: "Höhe", value: "40 cm" },
    { label: "Tiefe", value: "40 cm" },
    { label: "Schubladen", value: "2 Stück" },
    { label: "Auszug", value: "Vollauszug" },
    { label: "Funktion", value: "Soft-Close" },
...
const colorPrices: Record<string, number> = { white: 0, black: 0, blue: 15, green: 15, yellow: 15, orange: 15, red: 15 }

export default function MitDoppelschubladeProductPage() {
  ...
  const handleAddToCart = () => {
    addItem({ id: `${product.id}-${selectedColor}`, name: `${product.name} (${selectedColorData?.label})`, artNr: product.artNr, price: currentPrice })
    setAdded(true)
  ...
  return (
    <main className="min-h-screen bg-white">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16">
          <div className="relative">
            <div className="absolute top-4 left-4 z-20">
              <Badge className="bg-purple-600 text-white border-0 px-3 py-1.5 text-xs font-semibold tracking-wide">PREMIUM</Badge>
            </div>
            <div className="aspect-square bg-gradient-to-br from-gray-50 to-gray-100 overflow-hidden relative">
              <Product3DPreview moduleType={product.moduleType} color={selectedColor} width={80} />
          ...
          <div className="flex flex-col">
            ...
            {colorPrices[selectedColor] > 0 && <p className="text-sm text-gray-500 mt-1">Grundpreis {product.price.toFixed(2)} EUR + {colorPrices[selectedColor].toFixed(2)} EUR Farbaufschlag</p>}
            ...
            <div className="mt-8">
              ...
              <div className="flex flex-wrap gap-3">
                {availableColors.map((color) => (
                  <button key={color.id} onClick={() => setSelectedColor(color.id)} className={`group relative flex items-center gap-3 px-4 py-3 border-2 transition-all ${selectedColor === color.id ? "border-gray-900 bg-gray-50" : "border-gray-200 hover:border-gray-400"}`}>
                    <div className="w-6 h-6 border border-gray-300 shadow-inner" style={{ backgroundColor: color.hex }} />
                    <span className={`text-sm font-medium ${selectedColor === color.id ? "text-gray-900" : "text-gray-600"}`}>{color.label}</span>
          ...
          {activeTab === "specs" && (
            <div className="max-w-2xl mx-auto bg-white shadow-sm">
              <div className="divide-y divide-gray-100">
                {product.specs.map((spec, index) => (<div key={index} className="flex items-center justify-between px-6 py-5"><span className="text-gray-600">{spec.label}</span><span className="font-semibold text-gray-900">{spec.value}</span></div>))}
              </div>
            </div>
          )}
        </div>
      ...
      <section className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="prose prose-lg max-w-none text-gray-600"><p>{product.description}</p></div>
      </section>

...
type Props = {
  ...
  onApplyColorToRow: () => void
  onApplyColorToColumn: () => void
  onApplyColorToAll: () => void
  onClearPanelColor: () => void
  onClearFrontColor: () => void
}

const colorHexMap: Record<ShelfColor, string> = {
  weiss: "#f5f5f5",
  schwarz: "#1a1a1a",
  blau: "#2563eb",
  gruen: "#16a34a",
    ...
    <div className="flex w-96 flex-col border-l border-neutral-700 bg-neutral-800">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="border-b border-neutral-700 p-4">
          ...
          {/* Paint Mode Toggle */}
          <Tabs value={paintMode} onValueChange={(v) => onPaintModeChange(v as PaintMode)} className="mb-4">
            <TabsList className="grid w-full grid-cols-2 bg-neutral-700">
              <TabsTrigger value="panels" className="data-[state=active]:bg-neutral-600">
                Böden/Wände
              </TabsTrigger>
              <TabsTrigger value="fronts" className="data-[state=active]:bg-neutral-600">
                Fronten
              </TabsTrigger>
            </TabsList>
          </Tabs>
          ...
          <div className="mb-4">
            <p className="mb-2 text-xs text-neutral-400">
              Aktive Farbe für {paintMode === "panels" ? "Böden/Wände" : "Fronten"}:
            </p>
            <div className="flex flex-wrap gap-2">
              {allColors.map((color) => (
                <button
                  ...
                  className={cn(
                    ...
                    activeColor === color.id
                      ? "border-white ring-2 ring-white ring-offset-1 ring-offset-neutral-800"
                      : "border-neutral-600 hover:border-neutral-400",
                  )}
                  style={{ backgroundColor: color.color }}
                  title={color.label}
          ...
          {/* Selected Cell Info */}
          {selectedCell && (
            <div className="mb-4 rounded bg-neutral-700 p-2 text-xs">
              <p className="text-neutral-300">
                Ausgewählte Zelle: Zeile {selectedCell.row + 1}, Spalte {selectedCell.col + 1}
              </p>
              {config.cellStyles[`c-${selectedCell.row}-${selectedCell.col}`] && (
                <div className="mt-1 text-neutral-400">
                  {config.cellStyles[`c-${selectedCell.row}-${selectedCell.col}`].panelColor && (
                    <span className="mr-2">
                      Panel: {config.cellStyles[`c-${selectedCell.row}-${selectedCell.col}`].panelColor}
                    </span>
                  )}
                  {config.cellStyles[`c-${selectedCell.row}-${selectedCell.col}`].frontColor && (
                    <span>Front: {config.cellStyles[`c-${selectedCell.row}-${selectedCell.col}`].frontColor}</span>
                  )}
                </div>
              )}
            </div>
          ...
          <div className="space-y-2">
            <Button
              variant="outline"
              size="sm"
              className="w-full justify-start border-neutral-600 bg-neutral-700 text-neutral-100 hover:bg-neutral-600"
              onClick={onApplyColorToRow}
          ...
          {selectedCell && (
            <div className="mt-3 space-y-2">
              <p className="text-xs text-neutral-400">Überschreibung entfernen:</p>
              <div className="flex gap-2">
                <Button
                  variant="outline"
                  size="sm"
                  className="flex-1 border-neutral-600 bg-neutral-700 text-neutral-100 hover:bg-neutral-600"
                  onClick={onClearPanelColor}
                >
